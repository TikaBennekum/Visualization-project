...
# ============================================================
# 0. Locate all VTS files to animate
# ============================================================
...
files = sorted(glob.glob("mountain_backcurve40/output.*.vts"), key=extract_number)
...
vegetation_mapper.SetInputConnection(veg_contour.GetOutputPort())
...
smoke_contour_low, smoke_actor_low = make_iso_actor(
    grid,
    theta_name,
    low,
...
smoke_contour_mid, smoke_actor_mid = make_iso_actor(
    grid,
    theta_name,
    mid,
...
fire_contour_hi, fire_actor_hi = make_iso_actor(
    grid,
    theta_name,
    hi,
...
fire_contour_higher, fire_actor_higher = make_iso_actor(
    grid,
    theta_name,
    higher,
...
fire_contour_very_hi, fire_actor_very_hi = make_iso_actor(
    grid,
    theta_name,
    very_hi,
...
# --------------------------------------
# 8. ANIMATION LOOP — update data + render + save PNG
# --------------------------------------
# Find all VTS files for the animation
for frame_id, fname in enumerate(files):
    ...
    fire_contour_very_hi.SetInputData(grid)
    slice0.SetInputData(grid)
